...
package com.lew.common.thread;

import com.lew.common.util.SystemClock;
...
public class RejectedPolicyWithReport implements RejectedExecutionHandler {
    ...
    @Override public void rejectedExecution(Runnable r, ThreadPoolExecutor e) {
        try {
            ...
            String msg = MessageFormat.format(
                "{0}{1}core pool size:{2}, current pool size:{3}, queue wait size:{4}, active count:{5}, completed "
                    + "task count:{6}, "
                    + "task count:{7}, largest pool size:{8}, max pool size:{9}, keep alive time:{10}, is "
                    + "shutdown:{11}, is terminated:{12}, " + "thread name:{13}{14}", System.lineSeparator(), title,
                e.getCorePoolSize(), e.getPoolSize(), e.getQueue().size(), e.getActiveCount(),
                e.getCompletedTaskCount(), e.getTaskCount(), e.getLargestPoolSize(), e.getMaximumPoolSize(),
    ...
    private void threadDump() {
        ...
        pool.execute(() -> {
            ...
            lastPrintTime = SystemClock.now();
        });
        pool.shutdown();
    }

    @SuppressWarnings("all") private String getThreadDumpString(ThreadInfo threadInfo) {
        StringBuilder sb = new StringBuilder(
            "\"" + threadInfo.getThreadName() + "\"" + " Id=" + threadInfo.getThreadId() + " " + threadInfo
                .getThreadState());
